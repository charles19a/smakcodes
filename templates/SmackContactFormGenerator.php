...
if (!isset($_SESSION)) 
	...
	{
		if(file_exists( SM_LB_PRO_DIR . "captcha/recaptchalib.php" )){
			require_once( SM_LB_PRO_DIR . "captcha/recaptchalib.php" );
		}
	}
...
{
	global $module_options, $module, $assignedto, $check_duplicate, $update_record,$formattr, $attrname;
...
function callCurlPRO( $formtype )
{
	...
	global $formattr;
	global $attrname;
	global $module_options, $module , $isWidget, $assignedto, $check_duplicate, $update_record, $sendmail;
	$successfulAttemptsOption=[];
	$config_field_label = $total_config_fields = [];
	$plugin_dir=SM_LB_PRO_DIR;
	$activated_crm = get_option( 'WpLeadBuilderProActivatedPlugin' );	
	if($activated_crm == 'wpzohopro'){
		if(isset($post['Email_Opt_Out']) && $post['Email_Opt_Out'] == 'on'){
			$post['Email_Opt_Out'] = true;
		}
	}
	...
	if(isset($data) && $data) {
		...
		if(isset($config_fields['submit_count']) || isset($config_fields['success_count'])){
			$successfulAttemptsOption['total'] =  $config_fields['submit_count'];
			$successfulAttemptsOption['success'] = $config_fields['success_count'];
		}
		$total=0;
		$success=0;
		if(!empty($successfulAttemptsOption)){
			if(!isset($successfulAttemptsOption['total']) && isset($successfulAttemptsOption['success'] ))
			{
		...
		$total++;
		$contenttype = "\n";
		if(isset($config_fields['fields']) && is_array($config_fields['fields'])){
		...
		if(isset($config_fields['module']) && preg_match("/{$config_fields['module']} entry is added./",$data)) {
			$success++;
			...
			else if(isset( $config_fields['success_message'] ) && ($config_fields['success_message'] != "") )
			{
		...
		else
		{
			$successfulAttemptsOption['total'] = $total;
			$successfulAttemptsOption['success'] = $success;
			if( isset($config['emailcondition']) && ( ($config['emailcondition'] == 'failure') || ($config['emailcondition'] == 'both')) )
			{
				$sendmail = mailsendPRO( $config,$activatedplugin,$formtype, $pageurl, "Failure" , $contenttype );
			}
...
function normalContactFormPRO($module, $config_fields, $module_options , $formtype , $thirdparty)
{
	...
	for($i=0; $i<count($config_fields);$i++) {
		...
		$postsan = isset($_POST[$config_fields[$i]['name']]) ? sanitize_text_field($_POST[$config_fields[$i]['name']]) : '';
		if( $config_fields[$i]['publish']==1)
		{
			...
			{
				$config_pick = $config_fields[$i]['type']['picklistValues'];
				if (is_array($config_pick)) {
					$picklist_count = count($config_fields[$i]['type']['picklistValues']);
				}
				else{
					$picklist_count = '0';
				...
				for($j=0 ; $j<$picklist_count ; $j++)
				{
					...
					elseif($activatedplugin=='wpzohopro'){
						$content2 .= "<option id='{$config_fields[$i]['name']}' value='{$config_fields[$i]['type']['picklistValues'][$j]['display_value']['display_value']}'>{$config_fields[$i]['type']['picklistValues'][$j]['actual_value']['actual_value']}</option>";
					} 
					else {
						$content2 .= "<option id='{$config_fields[$i]['name']}' value='{$config_fields[$i]['type']['picklistValues'][$j]['value']}'>{$config_fields[$i]['type']['picklistValues'][$j]['label']}</option>";
					}
			...
			{
				$configfieldname = isset($config_fields[$i]['name']) ? $config_fields[$i]['name'] : '';
				$content1.="<td><input type='text' class='others{$M} smack_post_fields' name='{$configfieldname}' id='{$module_options}_{$configfieldname}' value='" . $postsan . "'/><br/><span class='smack_field_error' id='".$configfieldname."error{$_SESSION["generated_forms"]}'></span></td></tr>";
				$count_selected++;
			}
		}
	}
