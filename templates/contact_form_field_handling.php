...
function contact_forms_example()
{
			...
			foreach ( $data_array as $key => $value ) {
				...
				if ( is_array( $data_array[ $key ] ) ) {
					...
					case 'wpzohopro':
						...
						// if($contact_form_field_type[$i] ==='select'){
						// 	if(is_array($data_array[ $key ])){
                        //     // $data_array[ $key ] = json_encode($data_array[ $key ]);                            
						// }} 	
						break;
	...
	else
	{
		...
		$smackfieldName = $wpdb->get_results(
			$wpdb->prepare("
				SELECT a.field_name, a.field_values, a.field_type
				FROM wp_smackleadbulider_field_manager AS a
				JOIN wp_smackleadbulider_form_field_manager AS b
				JOIN wp_smackthirdpartyformfieldrelation AS c
				ON b.field_id = a.field_id
				AND c.smackfieldid = b.rel_id
				WHERE thirdpartyformid = %d
			", $post_id),
			ARRAY_A
		);
