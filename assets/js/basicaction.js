...
jQuery(document).ready(function () {
	...
	$(".form-submission-details > .wp-list-table").wrap("<div class='form-record-history'></div>");
	...
	if (module_own == 'Members') {
		jQuery(".module_owner").html("Assign Members to owner");
		jQuery("#hide_convert").addClass("disabledbutton");
	}
...
function formrecord_history(Action, Module, shortcode, plugin) {
	if (Action == "formrecord") {
		...
		jQuery.ajax({
			...
			success: function (data) {
				...
				var onAction = data_array.onAction;
				if (Action == 'formrecord') {
					window.location = "admin.php?page=lb-formhistory&__module=ManageShortcodes&__action=ManageFields&onAction=" + onAction + "&crmtype=" + crmtype + "&module=" + module + "&formrecord=" + shortcode + "";
				} else {
					window.location = "admin.php?page=lb-crmforms";
				}
...
function selectedPlugPRO(thiselement) {
	...
	if (get_config == 'no') {
		...
		jQuery.confirm({
			title: '',
			content: '<center>Switching to another CRM Will make Your old ShortCodes Disabled</center>',
			buttons: {
				formSubmit: {
					text: 'confirm',
					btnClass: 'popup_confirm_button pull-right',
					action: function () {
						var pluginselect_value;
				...
				cancel: {
					text: 'cancel',
					btnClass: 'popup_cancel_button pull-left',
					action: function () {
...
function enableWPUserAutoSync(id) {
	/*if( id == "enableAutoSync" )
	  ...
	  jQuery('#enableAutoSync').prop('checked' , true );
	  jQuery('#disableAutoSync').prop( 'checked' , false );
	  }
	  else
	  {
	  ...
	  jQuery('#enableAutoSync').prop('checked' , false );
	  jQuery('#disableAutoSync').prop( 'checked' , true ); 
	  }*/


	if (document.getElementById("enableAutoSync").checked == true) {
		jQuery("#OneTimeSync").prop('disabled', true);
		var autosync_val = "On";
	} else {
		jQuery("#OneTimeSync").prop("disabled", false);
...
function selectedcustomPRO(plugname) {
	jQuery.confirm({
		title: '',
		content: '<center><p style="font-size:19px">Do you want to change the plugin?</p></center>',
		buttons: {
			formSubmit: {
				text: 'confirm',
				btnClass: 'popup_confirm_button pull-right',
					...
					jQuery.ajax({
						...
						success: function (data) {
							if (data == "yes") {
								...
								jQuery("#custom_fields").val(old_custom_plugin);
							}
						},
						error: function (errorThrown) {
							console.log(errorThrown);
...
function save_captcha_key() {
	...
	console.log("value", emailcondition);
...
function SaveCheckPRO(siteurl, module, option, shortcode, onAction) {
	...
	for (i = 0; i < chkBx_count; i++) {
		...
		//var order=document.getElementById('select'+i).value;
		labelArray.push(Label);
		//orderArray.push(parseInt(order));
	}
...
//jQuery(".save-salesforce-config").click(function(e){
function saveSalesForceAPICredentials() {
	...
	jQuery.ajax({
		...
		success: function (data) {
			...
			if (data.success === true) {
				window.location.href = "https://login.salesforce.com/services/oauth2/authorize?response_type=code&client_id=" + data.client_id + "&redirect_uri=" + data.callback;
			}
		}, error: function (data) {
			console.log(data);
			return false;
