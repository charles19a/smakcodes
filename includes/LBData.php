...
class CaptureData {
	...
	function DeleteFields($crmtype , $module , $check_deleted_fields)
	{
		...
		foreach($check_deleted_fields as $del_key => $del_value)
		{
			...
			$getfields = isset($get_field_id[0]) ? $get_field_id[0]->field_id : '';
			$crm_field_id = $getfields;
	...
	function updateFormSubmitStatuses( $submit_parameters , $shortcodename )
	{
		...
		$wpdb->query("update wp_smackleadbulider_shortcode_manager set submit_count = '{$submit_parameters['total']}' , success_count = '{$submit_parameters['success']}' , failure_count = '{$submit_parameters['failure_count']}' where shortcode_name = '$shortcodename'");
	}
		...
		foreach( $get_shortcodes as $key => $shortcodedata )
		{
			...
			if( $crm_type == $shortcodedata->crm_type && $module_type == $module )
			{
				if(count($fields) == 0)
				{
					...
					$wpdb->get_results( "insert into wp_smackleadbulider_form_field_manager( field_id , shortcode_id , display_label , custom_field_type , custom_field_values , wp_field_mandatory , form_field_sequence , state ) VALUES ('$field_id', '$shortcode_id' , '$field_label', '$field_type', '$field_values' , $field_mandatory , $field_sequence , $publish )" );
				}
				else 
				{
					$state = "";
					...
					$wpdb->get_results("update wp_smackleadbulider_form_field_manager set wp_field_mandatory = '$field_mandatory'  where rel_id = '{$rel_id}'");
	...
	function formShorcodeManager($shortcodedata , $mode = "create")
	{
		...
		$first_userid = isset($get_userslist['id'][0]) ? $get_userslist['id'][0] : '';
		...
		{
			$wpdb->update( 'wp_smackleadbulider_shortcode_manager' , array( 'form_type' => "$form_type" , 'assigned_to' => "$assigned_to", 'error_message' => "$error_message" , 'success_message' => "$success_message" , 'is_redirection' => $is_redirection , 'url_redirection' => $url_redirection, 'google_captcha' => $google_captcha , 'duplicate_handling' => "$duplicate_handling") , array( 'shortcode_name' => "$shortcode_name" ) );
	...
	function formfields_settings($shortcode_name)
	{
		...
		if(isset($_REQUEST['crmtype'])) {
			...
			$field = $wpdb->get_results( "select * from wp_smackleadbulider_field_manager fm join wp_smackleadbulider_form_field_manager ffm ON ffm.field_id = fm.field_id join wp_smackleadbulider_shortcode_manager sm ON sm.shortcode_id = ffm.shortcode_id where sm.shortcode_name='{$shortcode_name}' group by fm.field_name order by ffm.form_field_sequence" );
		}
		$i = 0;
		$crmFields = array();
		foreach($field as $newfields) 
		{
			...
			if( $newfields->field_mandatory == 1 )
				$crmFields['fields'][$i]['mandatory'] = 2;
			else
	...
	function getFormSettings( $shortcodename = "" )
	{
		...
		if( !empty($shortcodename) ) {
			$query .= " WHERE shortcode_name = %s";
			$results = $wpdb->get_results( $wpdb->prepare($query, $shortcodename) );
		} else {
			$results = $wpdb->get_results($query);
		}

		if( !empty($shortcodename) && count($results) > 0 ) {
			return $results[0];
		} else {
			return $results;
		}
