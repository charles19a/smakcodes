...
class PROFunctions{
	...
	public function checkEmailPresent( $module , $email )
	{
		...
		$get_entries_count_parameters = array(
			...
			'query' => "{$module_table_name}.id in (SELECT eabr.bean_id FROM email_addr_bean_rel eabr JOIN email_addresses ea ON (ea.id = eabr.email_address_id) WHERE eabr.deleted=0 AND ea.email_address='$email')",
