...
$sales_callback_url = site_url().'/wp-admin/admin.php?page=lb-crmconfig';
  ...
  if(isset( $_REQUEST['code'] ) && (sanitize_text_field($_REQUEST['code']) != '') && !isset($config['id_token']) )
  {
    ...
    if (!empty($access_token)) {
      ...
      $config['refresh_token'] = !empty($refresh_token) ? $refresh_token : $config['refresh_token'];
      $config['api_domain'] = isset($response['api_domain']) ? $response['api_domain'] : "";
      ...
      <form id="smack-salesforce-settings-form" action="<?php echo esc_url($_SERVER['REQUEST_URI']); ?>" method="post">
        ...
        <div class="mt30">
          <div class="form-group col-md-12 ml15">
            <label id="inneroptions" class="leads-builder-heading" style="margin-left:0">Zoho Bigin Settings</label>
          </div>
        </div>
              ...
              <div style="position:relative;top:-20px;margin-left:197px;">
                <div class="tooltip">
                  <?php echo $help ?> <span class="tooltipPostStatus">
                    <h5>Client ID</h5>Get the Client ID from your Zoho Bigin account.
                  </span>
                </div>
              </div>
            </div>
              ...
              <div style="position:relative;top:-20px;margin-left:197px;">
                <div class="tooltip">
                  ...
                  <span class="tooltipPostStatus" style="width:330px;">
                    <h5>Client Secret</h5>Get the Client Secret from your Zoho Bigin account.
                  </span>
                </div>
              </div>
            </div>
        ...
        <input type="hidden" id="leads_fields_tmp" name="leads_fields_tmp"
          value="smack_wpzohobiginpro_lead_fields-tmp">
        <input type="hidden" id="contact_fields_tmp" name="contact_fields_tmp"
          value="smack_wpzohobiginpro_contact_fields-tmp">
        <div class="col-md-12">
